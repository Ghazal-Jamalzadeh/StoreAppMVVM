...
class ShippingViewModel @Inject constructor(private val repository: ShippingRepository) : ViewModel() {
    ...
    //shipping
    private val _shippingLiveData = MutableLiveData<MyResponse<ResponseShipping>>()
    val shippingLiveData: LiveData<MyResponse<ResponseShipping>> = _shippingLiveData

    //--- api call ---//
    fun getShipping() = viewModelScope.launch {

        _shippingLiveData.value = MyResponse.Loading()
